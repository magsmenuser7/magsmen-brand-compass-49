...
export const ResultsPage = ({ auditData, onStartOver }: ResultsPageProps) => {
  ...
  const cappedScore = Math.min(Math.round(auditData.score), 100);
  ...
  const getScoreIcon = (score: number) => {
    if (score > 75) return <CheckCircle className="h-8 w-8 text-green-600" />;
    if (score > 50) return <AlertTriangle className="h-8 w-8 text-orange-600" />;
  ...
  const categoryScores = [
    { name: 'Design Quality', score: Math.min(auditData.designQuality * 10, 100), icon: <Star className="h-5 w-5" /> },
    { name: 'Communication', score: Math.min(auditData.communicationClarity * 10, 100), icon: <MessageCircle className="h-5 w-5" /> },
    { name: 'Strategy Alignment', score: Math.min(auditData.strategyAlignment * 10, 100), icon: <Target className="h-5 w-5" /> },
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          <div className="text-center mb-6">
            <div className={`text-6xl font-bold ${getScoreColor(cappedScore)}`}>
              {cappedScore}%
            </div>
            <Progress value={cappedScore} className="mt-4 h-3" max={100} />
          </div>
          ...
          <div className="space-y-4">
            {categoryScores.map((category, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-24">
                    <Progress value={category.score} className="h-2" max={100} />
                  </div>
